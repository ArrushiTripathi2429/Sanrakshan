...
const sevStyles = { high:{dot:"#ef4444"}, medium:{dot:"#f59e0b"}, low:{dot:"#22c55e"} };
...
function ChartsSection({ issues }) {
  ...
  const trendData = Array.from({length:7},(_,i)=>{
    ...
    const count = issues.filter(x=>x.createdAt?.toDate&&x.createdAt.toDate().toDateString()===d.toDateString()).length;
    return {label,count};
  });
...
export default function AdminPage() {
    ...
    const unsub=onSnapshot(q,snap=>{
      ...
      docs.filter(i=>!i.assigned).forEach(i=>{
        VILLAGES_DATA.forEach(v=>{
          if(i.location?.toLowerCase().includes(v.name.toLowerCase())||
  ...
  const addMarker=(L,map,v)=>{
    ...
    if(v.issues>0){
      pulseMarkersRef.current[v.id]=L.circleMarker([v.lat,v.lng],{radius:r+10,color:"#ef4444",fillColor:"#ef4444",fillOpacity:0.05,weight:1,opacity:0.3}).addTo(map);
    }
  ...
  const openModal  = vid=>{setForm(f=>({...f,village:String(vid??1)}));setModal({open:true,villageId:vid});};
  ...
  const submitIssue=async()=>{
    ...
    try{
      ...
      await addDoc(collection(db,"reports"),{
        title:desc.slice(0,55),village:v.name,villageId:vid,location:v.name,
        category:form.cat,severity:form.sev,status:"pending",
        assigned:false,assignedTo:null,
        fieldWorkerId:"admin",fieldWorkerName:"Admin",
  ...
  const doAssign=async()=>{
    ...
    setAssigning(true);
    try{
      await updateDoc(doc(db,"reports",selIssue.id),{assigned:true,assignedTo:selVol.name,status:"assigned"});
      showToast("🤝",`${selVol.name} deployed`);
      setSelIssue(null);setSelVol(null);
    }catch(e){console.error(e);}
  ...
  const filteredVillages = villages.filter(v=>v.name.toLowerCase().includes(search.toLowerCase())||v.hi.includes(search));
  ...
  return (
    <>
        ...
        <main style={{flex:1,overflowY:"auto",padding:"28px 32px",maxWidth:1400,width:"100%",margin:"0 auto"}}>
          ...
          <div className="adm-fade adm-2" style={{marginBottom:28}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
              ...
              <button onClick={()=>openModal(null)} style={{background:"rgba(239,68,68,0.12)",border:"1px solid rgba(239,68,68,0.3)",color:"#f87171",fontSize:"0.75rem",fontWeight:600,padding:"8px 18px",borderRadius:10,cursor:"pointer",fontFamily:"'Outfit',sans-serif"}}>
                + Report Incident
              </button>
            </div>
          ...
          <div style={{background:"rgba(255,255,255,0.015)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:18,overflow:"hidden",marginBottom:20}}>
            <div style={{padding:"18px 24px",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <span style={{fontSize:"0.65rem",padding:"3px 10px",borderRadius:100,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.2)",color:"#f87171"}}>{issues.filter(i=>!i.assigned).length} unresolved</span>
            </div>
            {issues.length===0?(
              ...
              <div style={{padding:"12px 16px",display:"flex",flexDirection:"column",gap:8}}>
                {issues.map(issue=>(
                  <div key={issue.id} onClick={()=>!issue.assigned&&setSelIssue(issue)} style={{padding:"16px 20px",borderRadius:14,position:"relative",overflow:"hidden",cursor:issue.assigned?"default":"pointer",border:selIssue?.id===issue.id?"1px solid rgba(103,232,249,0.3)":"1px solid rgba(255,255,255,0.06)",background:issue.assigned?"rgba(255,255,255,0.01)":selIssue?.id===issue.id?"rgba(103,232,249,0.05)":"rgba(255,255,255,0.02)",opacity:issue.assigned?0.45:1,transition:"all 0.2s"}}>
                    <div style={{position:"absolute",top:0,left:0,width:3,height:"100%",background:sevStyles[issue.severity]?.dot||"#86efac",borderRadius:"3px 0 0 3px"}}/>
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:10,fontSize:"0.72rem",color:"rgba(232,245,233,0.35)",flexWrap:"wrap"}}>
                          ...
                          {issue.assigned&&<><span style={{opacity:0.4}}>·</span><span style={{color:"#86efac"}}>✓ {issue.assignedTo}</span></>}
                        </div>
                      </div>
                      <span style={{fontSize:"0.62rem",fontWeight:700,padding:"3px 10px",borderRadius:6,textTransform:"uppercase",letterSpacing:"0.06em",flexShrink:0,background:issue.severity==="high"?"rgba(239,68,68,0.12)":issue.severity==="medium"?"rgba(245,158,11,0.12)":"rgba(34,197,94,0.12)",color:issue.severity==="high"?"#f87171":issue.severity==="medium"?"#fbbf24":"#86efac",border:`1px solid ${issue.severity==="high"?"rgba(239,68,68,0.25)":issue.severity==="medium"?"rgba(245,158,11,0.25)":"rgba(34,197,94,0.25)"}`}}>
                        {issue.severity}
                      </span>
                    </div>
                  </div>
          ...
          {/* VOLUNTEERS */}
          <div style={{background:"rgba(255,255,255,0.015)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:18,overflow:"hidden",marginBottom:24}}>
            <div style={{padding:"18px 24px",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{padding:"12px 16px",display:"flex",flexDirection:"column",gap:8}}>
              {volunteers.length===0?(
                <div style={{padding:"32px 24px",textAlign:"center"}}>
                  <div style={{fontSize:"1.8rem",marginBottom:10,opacity:0.3}}>👤</div>
                  <div style={{fontSize:"0.82rem",color:"rgba(232,245,233,0.22)",lineHeight:1.6}}>No volunteers yet.<br/>They'll appear here once they sign in.</div>
                </div>
              ):volunteers.map(vol=>(
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:"flex",gap:5}}>
                      {(vol.skills||[vol.email]).filter(Boolean).map(s=>(
                        <span key={s} style={{fontSize:"0.62rem",padding:"2px 8px",borderRadius:4,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",color:"rgba(232,245,233,0.4)"}}>{s}</span>
                      ))}
                    </div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <div style={{width:7,height:7,borderRadius:"50%",background:vol.avail?"#86efac":"#fbbf24",boxShadow:vol.avail?"0 0 8px #86efac":"none"}}/>
                    <span style={{fontSize:"0.65rem",color:vol.avail?"#86efac":"#fbbf24"}}>{vol.avail?"Available":"Busy"}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* ASSIGN BAR */}
          <div className="adm-fade adm-4" style={{position:"sticky",bottom:20,background:"rgba(8,14,10,0.9)",backdropFilter:"blur(20px)",border:"1px solid rgba(134,239,172,0.12)",borderRadius:16,padding:"16px 24px",display:"flex",alignItems:"center",gap:20,boxShadow:"0 8px 40px rgba(0,0,0,0.5)"}}>
            <div style={{flex:1}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{padding:"4px 12px",borderRadius:8,background:selIssue?"rgba(103,232,249,0.08)":"rgba(255,255,255,0.03)",border:`1px solid ${selIssue?"rgba(103,232,249,0.2)":"rgba(255,255,255,0.06)"}`,color:selIssue?"#67e8f9":"rgba(232,245,233,0.2)",fontSize:"0.75rem"}}>
                  {selIssue?(selIssue.title||selIssue.description)?.slice(0,30)+"...":"Select an issue"}
                </span>
                <span style={{color:"rgba(232,245,233,0.2)"}}>→</span>
                <span style={{padding:"4px 12px",borderRadius:8,background:selVol?"rgba(134,239,172,0.08)":"rgba(255,255,255,0.03)",border:`1px solid ${selVol?"rgba(134,239,172,0.2)":"rgba(255,255,255,0.06)"}`,color:selVol?"#86efac":"rgba(232,245,233,0.2)",fontSize:"0.75rem"}}>
                  {selVol?selVol.name:"Select a responder"}
                </span>
              </div>
            </div>
            <button disabled={!selIssue||!selVol||assigning} onClick={doAssign} style={{padding:"12px 28px",borderRadius:12,border:"none",background:selIssue&&selVol?"linear-gradient(135deg,#86efac,#4ade80)":"rgba(255,255,255,0.04)",color:selIssue&&selVol?"#080e0a":"rgba(232,245,233,0.2)",fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:"0.82rem",cursor:selIssue&&selVol?"pointer":"not-allowed",transition:"all 0.2s"}}>
      ...
      {/* MODAL */}
      {modal.open&&(
        <div style={{position:"fixed",inset:0,background:"rgba(8,14,10,0.88)",backdropFilter:"blur(12px)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{background:"#0d1f12",border:"1px solid rgba(134,239,172,0.12)",borderRadius:22,padding:36,width:"100%",maxWidth:420,boxShadow:"0 24px 80px rgba(0,0,0,0.6)"}}>
            <div style={{fontFamily:"'Fraunces',serif",fontWeight:700,fontSize:"1.3rem",color:"#f0fdf4",marginBottom:6}}>Report Incident</div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              {[
                {label:"Village", field:"village",options:villages.map(v=>({val:String(v.id),label:v.name}))},
                {label:"Category",field:"cat",    options:["Water","Health","Food","Safety","Education","Infrastructure"].map(c=>({val:c,label:c}))},
                {label:"Severity",field:"sev",    options:[{val:"high",label:"Urgent (High)"},{val:"medium",label:"Standard (Medium)"},{val:"low",label:"Low Priority"}]},
                ...
                <div key={field}>
                  ...
                  <select value={form[field]} onChange={e=>setForm(f=>({...f,[field]:e.target.value}))} style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:"10px 14px",color:"#f0fdf4",fontFamily:"'Outfit',sans-serif",fontSize:"0.85rem",outline:"none"}}>
              ...
              <div>
                ...
                <textarea rows={3} value={form.desc} onChange={e=>setForm(f=>({...f,desc:e.target.value}))} placeholder="Describe the situation..." style={{width:"100%",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,padding:"10px 14px",color:"#f0fdf4",fontFamily:"'Outfit',sans-serif",fontSize:"0.85rem",outline:"none",resize:"none"}}/>
              </div>
            </div>
            <div style={{display:"flex",gap:10,marginTop:28}}>
              <button onClick={closeModal} style={{flex:1,padding:12,borderRadius:12,border:"1px solid rgba(255,255,255,0.07)",background:"none",color:"rgba(232,245,233,0.35)",fontFamily:"'Outfit',sans-serif",fontSize:"0.82rem",cursor:"pointer"}}>Cancel</button>
              <button onClick={submitIssue} style={{flex:2,padding:12,borderRadius:12,border:"none",background:"linear-gradient(135deg,#ef4444,#dc2626)",color:"white",fontFamily:"'Outfit',sans-serif",fontWeight:700,fontSize:"0.82rem",cursor:"pointer"}}>Submit Alert</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* TOAST */}
      <div style={{position:"fixed",bottom:28,left:"50%",transform:`translateX(-50%) translateY(${toast.show?0:16}px)`,zIndex:3000,padding:"12px 22px",borderRadius:14,background:"rgba(13,31,18,0.95)",border:"1px solid rgba(134,239,172,0.15)",boxShadow:"0 8px 32px rgba(0,0,0,0.5)",display:"flex",alignItems:"center",gap:10,opacity:toast.show?1:0,pointerEvents:toast.show?"auto":"none",transition:"all 0.4s ease"}}>
