...
export default function AdminPage() {
  ...
  // villages with live issue counts derived from Firestore
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      // Recompute issue counts per village
      const counts = {};
      docs.filter(i => !i.assigned).forEach((i) => {
        // match by location string against village names
      ...
      // Update villages state + refresh markers
      setVillages((prev) => {
        const updated = prev.map((v) => ({
          ...v,
          issues: counts[v.id] || 0,
        }));
        // Refresh markers after state update
  ...
  const doAssign = async () => {
    ...
    try {
      ...
      showToast("", `${selVol.name} deployed to "${selIssue.title?.slice(0, 30)}"`);
      setSelIssue(null);
      setSelVol(null);
    } catch (e) { console.error(e); }
  ...
  // ── RENDER (identical layout to your original) ──────────────────────────────
  return (
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .adm-fade-1 { animation-delay: 0.05s; } .adm-fade-2 { animation-delay: 0.12s; }
        .adm-fade-3 { animation-delay: 0.2s; }  .adm-fade-4 { animation-delay: 0.28s; }
      `}</style>
      ...
      <div style={{ minHeight: "100vh", background: "#080e0a", display: "flex", flexDirection: "column" }}>
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "28px 32px", maxWidth: 1400, width: "100%", margin: "0 auto" }}>
          ...
          <div className="adm-fade adm-fade-3" style={{ display: "flex", flexDirection: "column", gap: 20, marginBottom: 24 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 18, overflow: "hidden" }}>
              <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: "0.65rem", padding: "3px 10px", borderRadius: 100, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171" }}>
          ...
          {/* ASSIGN BAR */}
          <div className="adm-fade adm-fade-4" style={{ position: "sticky", bottom: 20, background: "rgba(8,14,10,0.9)", backdropFilter: "blur(20px)", border: "1px solid rgba(134,239,172,0.12)", borderRadius: 16, padding: "16px 24px", display: "flex", alignItems: "center", gap: 20, boxShadow: "0 8px 40px rgba(0,0,0,0.5)" }}>
            <div style={{ flex: 1 }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(8,14,10,0.88)", backdropFilter: "blur(12px)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ background: "#0d1f12", border: "1px solid rgba(134,239,172,0.12)", borderRadius: 22, padding: 36, width: "100%", maxWidth: 420, boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 28 }}>
              <button onClick={closeModal} style={{ flex: 1, padding: "12px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.07)", background: "none", color: "rgba(232,245,233,0.35)", fontFamily: "'Outfit', sans-serif", fontSize: "0.82rem", cursor: "pointer" }}>Cancel</button>
              <button onClick={submitIssue} style={{ flex: 2, padding: "12px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #ef4444, #dc2626)", color: "white", fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: "0.82rem", cursor: "pointer" }}>Submit Alert</button>
            </div>
          </div>
        </div>
      )}
